...
@section('content')
    <div class="mb-5 flex items-center gap-2">
        ...
        <span class="text-sm text-slate-500">New Client</span>
    </div>

    <div class="mx-auto max-w-xl rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <h1 class="font-heading text-xl font-bold text-brand-navy">New Client</h1>
        <p class="mt-0.5 text-sm text-slate-400">Name, address, and contact details.</p>

        <form method="POST" action="{{ route('admin.clientes.store') }}" class="mt-6 space-y-5">
            @csrf
