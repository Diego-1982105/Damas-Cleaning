...
@section('content')
    <div class="mb-5 flex items-center gap-2">
        ...
        <span class="text-sm text-slate-500">New Service</span>
    </div>

    <div class="mx-auto max-w-lg rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
        <h1 class="font-heading text-xl font-bold text-brand-navy">New Service</h1>
        <p class="mt-0.5 text-sm text-slate-400">Name and reference price (overridable per invoice).</p>

        <form method="POST" action="{{ route('admin.configuracion.servicios.store') }}" class="mt-6 space-y-5">
            @csrf
            <div>
            ...
            <div class="grid gap-4 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <p class="mt-1 text-xs text-slate-400">Lower = first in lists.</p>
                    @error('sort_order')
                        <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
                    @enderror
                </div>
